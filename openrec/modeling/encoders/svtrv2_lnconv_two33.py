...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Use PyTorch native Memory-Efficient Attention (fused kernel)
...
class CoordAttBlock(nn.Module):
    """Horizontal-focused Coordinate Attention Block for ALPR.

    Replaces O(N^2) Global Attention with O(N) coordinate-aware attention.
    Motivation for ALPR: License plates are strictly horizontal sequences.
    Global attention computes relationships between all (x,y) pairs indiscriminately.
    CoordAtt explicitly separates horizontal (W) and vertical (H) encoding,
    forcing the network to learn character sequences along the W axis.
    """
    def __init__(self, dim, mlp_ratio=4.0, drop=0.0, drop_path=0.0,
        ...
        self.norm_ca = nn.BatchNorm2d(mip)
    ...
    def forward(self, x):
        ...
        # Coordinate Attention (X and Y sequence awareness)
...
class SVTRStage(nn.Module):
    ...
    def __init__(self,
        ...
        if downsample:
            if mixer[-1] in ('Conv', 'FGlobalRe2D', 'HFA'):
